#!/bin/bash
#SBATCH --job-name=replace-metaworld-run-no-sc
...
#SBATCH --output=logs/no-sc-replace-metaworld-run-%x-%j.out
#SBATCH --error=logs/no-sc-replace-metaworld-run-%x-%j.err
...
python scripts/train_wan22_i2v_metaworld_external.py \
    --config configs/wan22_replace_no_shortcut.yaml \
