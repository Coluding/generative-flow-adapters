#!/bin/bash
#SBATCH --job-name=gfa-train-gatelow-noshortcut
...
# Keep identical to the shortcut-ON sibling (submit_train_adaln_gatelow.sh)
# so the A/B differs only in shortcut_anchor_prob.
...
# ---- run --------------------------------------------------------------------
# Zero-shortcut-weight control: identical to the gate-fixed gatelow baseline
# except shortcut_anchor_prob 0.6 -> 1.0 (shortcut term never fires). Pure
# action-conditioned flow matching. Run alongside submit_train_adaln_gatelow.sh.
# Vault ticket: 20_Tickets/experiments/exp-shortcut-zero-weight-control-run.md
python scripts/train_wan22_i2v_metaworld_external.py \
    --config configs/wan22/diffusion_wan22_avid_gatelow_noshortcut_metaworld.yaml \
    --hdf5 "$ds_path" --ckpt-dir ckpts/Wan2.2-TI2V-5B \
    --batch-size $BATCH_SIZE --num-windows 8 --max-area 589824 --steps 5000000 \
    --wandb-run-name gatelow-noshortcut-control "$@"
