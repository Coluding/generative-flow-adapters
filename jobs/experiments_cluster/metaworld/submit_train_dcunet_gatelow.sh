#!/bin/bash
#SBATCH --job-name=gfa-train-dcunet-gatelow
...
# matching the AVID reference comparison run. H100 has headroom over the 11M
# tier, but raising it breaks comparability with the AVID-adapter runs.
...
# ---- run --------------------------------------------------------------------
# DC-UNet capacity test: high-capacity DynamiCrafter 3D-UNet output adapter
# (scratch init, 11M tier) with the gate + optimizer confounds fixed. Direct
# test of capacity vs injection-mechanism. Config requires the _external script.
# Vault ticket: 20_Tickets/experiments/exp-adapter-dcunet-gatelow-capacity-run.md
python scripts/train_wan22_i2v_metaworld_external.py \
    --config configs/wan22/diffusion_wan22_dcunet_output_metaworld.yaml \
    --hdf5 "$ds_path" --ckpt-dir ckpts/Wan2.2-TI2V-5B \
    --batch-size $BATCH_SIZE --num-windows 8 --max-area 589824 --steps 5000000 \
    --wandb-run-name dcunet-gatelow-capacity "$@"
