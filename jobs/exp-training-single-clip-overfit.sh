#!/bin/bash
#SBATCH --job-name=gfa-overfit-single-clip
...
#SBATCH --output=logs/overfit-single-clip-%x-%j.out
#SBATCH --error=logs/overfit-single-clip-%x-%j.err
...
python scripts/train_wan22_i2v_metaworld_external.py \
    --config configs/diffusion_wan22_avid_xattn_i2v_metaworld_debug.yaml \
    --hdf5 "$ds_path" --ckpt-dir ckpts/Wan2.2-TI2V-5B \
    --batch-size $BATCH_SIZE "$@" --num-windows 1 --max-area 589824 --steps 5000000 \
    --wandb-run-name "overfit-single-clip" --overfit-index 0    
