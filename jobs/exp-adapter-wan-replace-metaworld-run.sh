#!/bin/bash
#SBATCH --job-name=replace-metaworld-run
...
#SBATCH --output=logs/replace-metaworld-run-%x-%j.out
#SBATCH --error=logs/replace-metaworld-run-%x-%j.err
...
python scripts/train_wan22_i2v_metaworld_external.py \
    ...
    --wandb-run-name replace-metaworld-run
