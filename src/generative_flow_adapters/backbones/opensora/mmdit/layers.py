"""MMDiT layer implementations.

Vendored from Open-Sora (https://github.com/hpcaitech/Open-Sora)
Original source: opensora/models/mmdit/layers.py
...
Modifications:
    - Made liger_kernel optional with standard RMSNorm fallback
    - Removed torch.compile decorator for broader compatibility
...
from generative_flow_adapters.backbones.opensora.mmdit.math import (
    attention,
    liger_rope,
    rope,
)

# Try to import liger kernel for optimized RMSNorm
try:
...
class EmbedND(nn.Module):
    """N-dimensional position embedding using RoPE rotation matrices."""
...
class LigerEmbedND(nn.Module):
    """N-dimensional position embedding using liger-format RoPE (cos/sin)."""
...
class MLPEmbedder(nn.Module):
    """MLP for embedding vectors (timesteps, CLIP vectors, etc.)."""
...
class FusedRMSNorm(RMSNorm):
    """Fused RMSNorm using liger kernel if available, otherwise standard."""

    def forward(self, x: Tensor) -> Tensor:
        if LIGER_AVAILABLE and LigerRMSNormFunction is not None:
...
class QKNorm(nn.Module):
    """Query-Key normalization as in SD3 paper."""
...
class SelfAttention(nn.Module):
    """Self-attention with optional fused QKV projection."""
...
class Modulation(nn.Module):
    """Adaptive modulation layer for DiT-style conditioning."""
...
class DoubleStreamBlockProcessor:
    """Processor for DoubleStreamBlock forward pass."""
    ...
    def __call__(
        ...
        if attn.img_attn.fused_qkv:
            ...
            img_q = rearrange(
                attn.img_attn.q_proj(img_modulated), "B L (H D) -> B L H D", H=attn.num_heads
            )
            img_k = rearrange(
                attn.img_attn.k_proj(img_modulated), "B L (H D) -> B L H D", H=attn.num_heads
            )
            img_v = rearrange(
                attn.img_attn.v_proj(img_modulated), "B L (H D) -> B L H D", H=attn.num_heads
            )
        ...
        if attn.txt_attn.fused_qkv:
            ...
            txt_q = rearrange(
                attn.txt_attn.q_proj(txt_modulated), "B L (H D) -> B L H D", H=attn.num_heads
            )
            txt_k = rearrange(
                attn.txt_attn.k_proj(txt_modulated), "B L (H D) -> B L H D", H=attn.num_heads
            )
            txt_v = rearrange(
                attn.txt_attn.v_proj(txt_modulated), "B L (H D) -> B L H D", H=attn.num_heads
            )
...
class DoubleStreamBlock(nn.Module):
    """Double-stream transformer block for joint image-text processing."""
...
class SingleStreamBlockProcessor:
    """Processor for SingleStreamBlock forward pass."""
...
class SingleStreamBlock(nn.Module):
    """Single-stream DiT block with parallel linear layers.

    Based on https://arxiv.org/abs/2302.05442 with adapted modulation interface.
    """
