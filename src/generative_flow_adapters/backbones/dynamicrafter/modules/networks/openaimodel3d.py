...
from generative_flow_adapters.backbones.dynamicrafter.basics import avg_pool_nd, conv_nd, linear, normalization, zero_module
from generative_flow_adapters.backbones.dynamicrafter.common import checkpoint
from generative_flow_adapters.backbones.dynamicrafter.models.utils_diffusion import timestep_embedding
from generative_flow_adapters.backbones.dynamicrafter.modules.attention import SpatialTransformer, TemporalTransformer
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        action_dims=7,
        adapter_condition_dim=None,
        adapter_condition_hidden_dim=None,
        ...
        self.extra_conditioned = extra_conditioned
        ...
        if adapter_condition_dim is not None and int(adapter_condition_dim) > 0:
            adapter_hidden_dim = int(adapter_condition_hidden_dim or adapter_condition_dim)
            self.adapter_condition_proj = nn.Sequential(
                linear(int(adapter_condition_dim), adapter_hidden_dim),
                nn.SiLU(),
                linear(adapter_hidden_dim, adapter_condition_output_dim),
            )
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not self.action_conditioned and adapter_embedding is None:
            ...
            elif adapter_embedding is not None:
                cond_emb = self._prepare_adapter_embedding(adapter_embedding, batch_size=b, frames=t)
            else:
                cond_emb = self.null_action_emb.repeat_interleave(repeats=t * b, dim=0)
    ...
    def _prepare_adapter_embedding(self, adapter_embedding, batch_size, frames):
        if adapter_embedding.dim() == 2:
            projected = self.adapter_condition_proj(adapter_embedding)
        ...
        if adapter_embedding.dim() == 3:
            if adapter_embedding.shape[0] != batch_size or adapter_embedding.shape[1] != frames:
                raise ValueError("adapter_embedding with rank 3 must have shape [batch, frames, cond_dim].")
            adapter_embedding = rearrange(adapter_embedding, "b t c -> (b t) c")
            return self.adapter_condition_proj(adapter_embedding)
        raise ValueError("adapter_embedding must have rank 2 or 3.")
